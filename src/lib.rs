pub mod util {
    ...
    /// 封装的post请求
    async fn response_for_post(url: &str, headers: HeaderMap, body: HashMap<String,String>) -> Result<Response, Box<dyn Error>>{
        let p = CLIENT
            .post(url)
            .headers(headers)
            .form(&body)
    ...
    fn get_download_header(id_str:&str, user:&mut UserSession) -> HeaderMap{
        ...
        header.insert(cookie_key.clone(), format_cookie_str(&user.token, &user.session).parse().unwrap());
        ...
        header.insert(CONTENT_TYPE, reqwest::header::HeaderValue::from_static("application/x-www-form-urlencoded"));
        header
    }
    /// 封装的cookie解析
    /// 自动更新session
    fn get_new_cookie(header: &HeaderMap, user:&mut UserSession) -> (Option<String>, Option<String>){
        ...
        for val in all {
            ...
            } else if let Some(COOKIE_) = reg_cookie.captures(str){
                if cookie.eq(&None) {
                    cookie = Some(
                        get_regex_one(COOKIE_)
                    );
    ...
    /// debug方法,暂留
    fn pring_xxx(f:&(Option<String>, Option<String>)){
        if let Some(s) = &f.0 {
            println!("1:'{}'",s);
        } else {
            println!("1 is none!")
    ...
    async fn do_home(user:&mut UserSession) -> Result<(), OsuMapDownloadError>{
        let mut header = HeaderMap::new();
        header.insert("cookie", format_cookie_str(&user.token, &user.session).parse().unwrap_or(HeaderValue::from(0)));
    ...
    async fn do_login(user:&mut UserSession) -> Result<(), OsuMapDownloadError>{
        ...
        header.insert(reqwest::header::CONTENT_TYPE, reqwest::header::HeaderValue::from_static("application/x-www-form-urlencoded"));
        header.insert(reqwest::header::COOKIE, format_cookie_str(&user.token, &user.session).parse().unwrap());
        ...
        match response.status() {
            StatusCode::OK => {
                let f = get_new_cookie(response.headers(), user);
                //pring_xxx(&f);
                Ok(())
            },
