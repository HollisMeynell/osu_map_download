...
/// A builder pattern struct for creating the UserSession.
/// It can be built by username & password or by previous saved query token.
...
impl UserSessionBuilder {
    /// Create UserSession from username and password. UserSession should be built by calling the
    /// login method.
    pub fn from_user<T: Into<String>, U: Into<String>>(username: T, password: U) -> Self {
        Self {
    ...
    /// Create UserSession from previous data. Data should be generated by
    /// UserSession::to_recoverable function.
    pub fn from_data<T: Into<String>>(data: T) -> Option<UserSession> {
        let data = data.into();
    ...
    pub async fn login() -> Result<UserSession> {
        todo!()
    }
}
...
impl UserSession {
    ...
    /// 通过保存的session数据恢复
    pub fn from_recoverable(data: &str) -> Option<UserSession> {
        let parts: Vec<&str> = data.split(",").collect();
        if parts.len() < 2 {
            return None;
        }
